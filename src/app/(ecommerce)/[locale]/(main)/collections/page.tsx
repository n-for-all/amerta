...
export default async function CollectionsPage({ params, searchParams }: Props) {
  ...
  try {
    ...
    const { docs: collections, totalPages } = await getCollections({ page, locale }, salesChannel);
        ...
        <main className="container px-4 py-16 mx-auto sm:py-20 lg:py-24">
          ...
          {collections.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20">
              ...
              <h3 className="mb-2 text-2xl font-semibold text-zinc-900 dark:text-white">{__("No Collections Yet")}</h3>
              <p className="text-center text-zinc-600 dark:text-zinc-400">{__("Check back soon for our exciting collections!")}</p>
              <Button asChild>
            ...
            <>
              <div className="flex flex-col items-center text-center py-14 lg:py-20">
                <h1 className="mt-5 text-3xl font-medium leading-none sm:text-4xl xl:text-5xl/none">
                  <span className="text-zinc-500">{__("Collections")}</span>
                  <br />
                </h1>
                <div className="max-w-xl mt-5 uppercase text-sm/6 text-zinc-600 dark:text-zinc-400">{__("Explore our curated collections of premium products.")}</div>
              </div>
      ...
      <div className="w-full min-h-screen bg-white dark:bg-zinc-950">
        <main className="container px-4 py-16 mx-auto sm:py-20 lg:py-24">
          <div className="text-center">
            <h1 className="text-2xl font-semibold text-zinc-900 dark:text-white">{__("Error loading collections")}</h1>
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  const { locale } = await params;
  const metaData = await generateStaticMeta({ pageName: "collectionsPage", locale, type: "collections" });
  return metaData;
}
