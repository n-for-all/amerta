...
import { ProductBrand, Product } from "@/payload-types";
...
import { BrandFilters } from "@/amerta/theme/components/BrandFilters";
...
import { getBrandBySlug } from "@/amerta/theme/utilities/get-brand-by-slug";
import { getProductsBy } from "@/amerta/theme/utilities/get-product-by";
import { SearchParams } from "next/dist/server/request/search-params";
import { getProductsCollections } from "@/amerta/theme/utilities/get-products-collections";

export const dynamic = "force-dynamic";

export default async function BrandPage({ params, searchParams }: { params: Promise<{ locale: string; slug: string }>; searchParams: Promise<SearchParams> }) {
  ...
  let brandDescription: any = {};

  const allOptions = await getAllProductOptions(locale);
  const collections = await getProductsCollections(locale);
      ...
      <main className="container">
        ...
        <div className="flex flex-col items-center text-center py-14 lg:py-20">
          ...
          <div className="max-w-xl mt-5 uppercase text-sm/6 text-zinc-600 dark:text-zinc-400">
            <RichText data={brandDescription} enableProse={false} enableGutter={false} locale={locale} />
          </div>
        </div>

        <BrandFilters totalProducts={totalProducts} currentProductCount={products.length} collections={collections} brands={[]} options={allOptions} />
        ...
        <div className="pt-10 pb-16 sm:pt-12 sm:pb-24">
          {totalProducts === 0 ? (
            <div className="flex flex-col items-center justify-center py-20">
              ...
              <p className="max-w-md mb-8 text-center text-zinc-600 dark:text-zinc-400">{printf(__("Sorry, there are currently no products in the %s brand. Check back soon or browse our other brands!"), brandName)}</p>
              <Button size={"lg"} variant="default" asChild>
                <Link href={getURL(`/brands`, locale)}>{__("Browse All Brands")}</Link>
              </Button>
            </div>
          ) : (
            <section>
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string; locale: string }> }): Promise<Metadata> {
  ...
  let page: ProductBrand | null = null;
  try {
    page = await getBrandBySlug(slug, locale);
  } catch {
  ...
  return generateMeta({ doc: page, type: "brands", locale });
}
