...
const LoginForm: React.FC<{
  ...
  logo,
  redirectTo,
  innerClassName = "flex flex-col items-center w-full max-w-sm px-6 py-12 border rounded-lg min-w-sm gap-y-4 bg-background border-zinc-200 dark:border-zinc-800",
  onLoginSuccess,
}) => {
    ...
    errorMessage = (
      <div>
        ...
        <Button variant="outline" size="sm" asChild>
          <Link
            href={getURL(
              `/resend-verification-email${allParams.trim() ? `?` : ""}${allParams}`,
              locale,
            )}
          >
        ...
        <div className="flex items-center justify-center h-full">
          <div className="flex flex-col items-center gap-6 lg:justify-start">
                  ...
                  {providers.map((provider) => {
                    ...
                    if (provider.type === "google") {
                      return (
                        ...
                        >
                          <svg
                            className={"w-6 h-5 mr-2 rtl:ml-2 rtl:mr-0"}
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <Link
                        ...
                        href={getURL(
                          `/recover-password${allParams.trim() ? `?` : ""}${allParams}`,
                          locale,
                        )}
                      >
                        {__("Forget password?")}
            ...
            <div className="flex justify-center gap-1 text-sm text-muted-foreground">
              ...
              <Link
                href={getURL(
                  `/create-account${allParams.trim() ? `?` : ""}${allParams}`,
                  locale,
                )}
