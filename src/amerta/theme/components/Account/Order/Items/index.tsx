  ...
  return (
    <div className="pt-5 pb-2 mt-6 mb-6 border-t border-zinc-200 dark:border-zinc-800">
      <div className="overflow-x-auto ">
        <div className="grid grid-cols-[minmax(250px,1fr)_120px_60px_140px] text-base font-semibold text-zinc-500 border-b border-zinc-200 dark:border-zinc-800 pb-2 mb-2">
          <div className="min-w-32">{__("Item")}</div>
        ...
        {order.items?.map((item: any, idx) => {
          ...
          return (
            <div key={idx} className="grid grid-cols-[minmax(250px,1fr)_120px_60px_140px] items-start py-3 border-b border-zinc-100 dark:border-zinc-900">
              <div className="flex flex-row items-center gap-6 min-w-32">
                {/* Product Image */}
                {productImage ? (
      ...
      <div className="grid grid-cols-[30px_minmax(0,1fr)_140px] items-center py-3 pb-5 border-b border-zinc-200 dark:border-zinc-800">
        ...
        <div className="text-right">
          {isFreeShipping ? <span className="mr-1 line-through text-zinc-400">{formatPrice(Number(shippingTotal), (payment?.currency || currency) as Currency, exchangeRate)}</span> : <span>{formatPrice(Number(shippingTotal), (payment?.currency || currency) as Currency, exchangeRate)}</span>}
        </div>
      </div>
