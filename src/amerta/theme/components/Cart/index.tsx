        ...
        <div className="fixed inset-0">
          <div className="absolute inset-0 overflow-hidden">
            <div className="fixed inset-y-0 right-0 flex max-w-full rtl:left-0 rtl:right-auto">
              <TransitionChild enter="transform transition ease-out duration-300 sm:duration-500" enterFrom="translate-x-full rtl:-translate-x-full" enterTo="translate-x-0" leave="transform transition ease-in duration-200 sm:duration-300" leaveFrom="translate-x-0" leaveTo="translate-x-full rtl:-translate-x-full">
                                ...
                                {cart.items?.map((item) => {
                                  ...
                                  return (
                                    <li key={item.product.id} className="flex py-4">
                                      {/* Product Image */}
                                      <div className="relative w-24 h-32 overflow-hidden rounded-md shrink-0">{(item.product.images?.[0] as any)?.url ? <ImageMedia alt={(item.product.images?.[0] as any)?.alt || item.product.title} src={(item.product.images?.[0] as any)?.url} fill imgClassName="object-contain w-full h-full" /> : <div className="w-full h-full bg-zinc-200 dark:bg-zinc-700" />}</div>

                                      <div className="flex flex-col flex-1 ms-4">
                                        {/* Product Name and Price */}
                                        <div className="flex justify-between font-medium">
                                          <h3 className="leading-tight">
                                            <Link href={`/en/products/${item.product.id}`} className="uppercase text-sm/6 text-zinc-900 dark:text-white">
                                              {item.product.title}
                                            </Link>
                                          </h3>
                                        ...
                                        {item.inStock === false && <p className="mt-1 text-xs text-red-600">{__("This item is out of stock.")}</p>}
                                        {/* Quantity and Remove */}
                                        <div className="flex items-center justify-between pt-2 mt-1 text-sm">
                                          <div className="inline-grid w-full grid-cols-1 max-w-16">
                                            <select value={item.quantity} onChange={(e) => handleQuantityChange(item.product.id, parseInt(e.target.value))} disabled={updatingItemId === item.product.id} className="col-start-1 row-start-1 appearance-none rounded-md bg-white py-0.5 ps-3 pe-8 text-xs/6 outline-1 -outline-offset-1 outline-zinc-900/10 focus:outline-1 dark:bg-zinc-800 dark:text-white dark:outline-white/10 disabled:opacity-50">
                                          ...
                                          <button type="button" onClick={() => handleRemoveItem(item.product.id)} disabled={updatingItemId === item.product.id} className="p-2 -m-2 font-medium cursor-pointer text-zinc-600 hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-white disabled:opacity-50" title={__("Remove item from cart")}>
                          ...
                          <section aria-labelledby="summary-heading" className="grid gap-4 py-4 mt-auto border-t shrink-0 border-zinc-900/10 dark:border-white/10">
                            ...
                            <div>
                              ...
                              <div className="grid grid-cols-2 gap-2 mt-5">
                                <Link href={getURL(`/cart`, locale)} onClick={onCloseDialog} className="relative isolate inline-flex shrink-0 items-center justify-center gap-x-2 rounded-full border border-zinc-900 px-5 py-3.5 sm:px-6 sm:py-4 text-sm/none uppercase text-zinc-950 transition-colors hover:bg-zinc-950/5 dark:border-white/40 dark:text-white dark:hover:bg-white/5">
                                  {__("View cart")}
                                </Link>

                                <Link href={getURL(`/checkout`, locale)} onClick={onCloseDialog} className="relative isolate inline-flex shrink-0 items-center justify-center gap-x-2 rounded-full border border-transparent bg-zinc-900 px-5 py-3.5 sm:px-6 sm:py-4 text-sm/none uppercase text-white transition-colors hover:bg-zinc-800 dark:bg-white dark:text-zinc-900 dark:hover:bg-zinc-100">
                                  {__("Check out")}
                                </Link>
                              </div>
                              ...
                              <div className="flex justify-center mt-4 text-sm text-center text-zinc-500">
                                <p className="text-xs uppercase text-sm/6">
                                  {__("or")} <Link href={getURL(`/`, locale)} onClick={onCloseDialog} className="text-xs font-medium uppercase text-zinc-900 text-sm/6 dark:text-white hover:underline">
