...
export const OrderSummary = ({ cart, exchangeRate, currency, locale, shippingCost, onCartUpdate, tax, loadingTaxes, qualifiesForFree, total, isSubmitting, isValid }: { cart: CartWithCalculations; exchangeRate: number; currency: Currency; locale?: string; shippingCost: number; onCartUpdate: (cart: CartWithCalculations) => void; orderTaxPercentage: number; tax: number; loadingTaxes: boolean; qualifiesForFree: boolean; total: number; isSubmitting: boolean; isValid: boolean }) => {
  ...
  return (
    <div className="sticky mt-10 lg:mt-0 top-24">
      ...
      <div className="relative sticky mt-5 bg-white border rounded-lg top-24 border-zinc-200 dark:bg-zinc-900 dark:border-zinc-800">
        ...
        <CouponForm showLabel={false} cart={cart} onCartUpdate={onCartUpdate} currency={currency} className={"px-6 border-t border-zinc-200 py-4 bg-zinc-50 mb-0 dark:bg-zinc-900 dark:border-zinc-800"} />
        <dl className="px-4 py-6 space-y-2 border-t border-zinc-200 sm:px-6 dark:border-zinc-800">
          ...
          <div className="flex items-center justify-between pt-6 border-t border-zinc-200 dark:border-zinc-800">
            ...
            <dd className="text-base font-medium text-zinc-900 dark:text-zinc-100">{formatPrice(total, currency, exchangeRate)}</dd>
          </div>
        </dl>
