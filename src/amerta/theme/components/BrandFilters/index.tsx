...
function PriceFilterDropdown({ min, max, currency, onApply, label, buttonLabel }: { min: string; max: string; currency: Currency; onApply: (min: string, max: string) => void; label: string; buttonLabel: string }) {
                  ...
                  <FilterDropdown key={option.id} label={option.name} count={searchParams.getAll(`opt_${option.id}`).length}>
                    <div className="p-4 grid grid-cols-4 gap-2 min-w-[200px]">
                      {option.images?.map((opt) => (
                        <ImageSwatch key={opt.id} image={(opt.image as ProductMedia)?.url || ""} label={opt.label || ""} noImageLabel={__("No image")} checked={isSelected(`opt_${option.id}`, opt.id || "")} onChange={() => updateFilter(`opt_${option.id}`, opt.id || "")} />
                      ))}
                    </div>
                  </FilterDropdown>
                );
...
function ImageSwatch({ noImageLabel, image, label, checked, onChange }: { noImageLabel: string; image: string; label: string; checked: boolean; onChange: () => void }) {
  return (
