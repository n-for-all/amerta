...
const serialize = (children?: Children, locale?: string): React.ReactNode[] => {
  return (
    children?.map((node, i) => {
      if (Text.isText(node)) {
      ...
      switch (node.type) {
        ...
        case "link":
          return (
            <CMSLink key={i} type={node.linkType === "internal" ? "reference" : "custom"} url={node.url} reference={node.doc as any} newTab={Boolean(node?.newTab)} locale={locale!}>
