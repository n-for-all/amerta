import { getCachedGlobal } from "@/amerta/utilities/getGlobals";
import { Config } from "payload";
import React, { ReactNode } from "react";
import type { Header as HeaderProps, Settings } from "@/payload-types";
import { Header } from "./blocks/common/Header/Component";
import { Footer } from "./blocks/common/Footer/Component";
import { EcommerceProvider } from "./providers/EcommerceProvider";
import { Toaster } from "./ui/toast";
import { getSalesChannel } from "./utilities/get-sales-channel";
import { LOCALES } from "@/amerta/localization/locales";
import { getDictionary } from "./utilities/translation";
import { getCurrentCurrency } from "./utilities/get-current-currency";
import { getCurrencies } from "./utilities/get-currencies";

interface Props {
  children: ReactNode;
  header?: ReactNode;
  locale: string;
}

const WhatsappChat = ({ url }) => {
  return (
    <a className="whatsapp" target="_blank" rel="noopener noreferrer" href={url}>
      <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full">
        <path d="M8.88595 7.16985C9.06891 7.17475 9.27175 7.18465 9.46474 7.61303C9.59271 7.89821 9.80829 8.42321 9.9839 8.85087C10.1206 9.18366 10.233 9.45751 10.2611 9.51356C10.3254 9.64156 10.365 9.78926 10.2809 9.96156C10.271 9.98188 10.2617 10.0013 10.2526 10.02C10.1852 10.16 10.1372 10.2597 10.0237 10.3899C9.97709 10.4435 9.9285 10.5022 9.88008 10.5607C9.79494 10.6636 9.71035 10.7658 9.63785 10.838C9.50924 10.9659 9.37563 11.1039 9.52402 11.3599C9.6725 11.6159 10.1919 12.4579 10.9587 13.1373C11.783 13.8712 12.4998 14.1805 12.8622 14.3368C12.9325 14.3672 12.9895 14.3918 13.0313 14.4126C13.2886 14.5406 13.4419 14.5209 13.5903 14.3486C13.7388 14.1762 14.2334 13.6001 14.4066 13.3441C14.5748 13.0881 14.7479 13.1275 14.9854 13.2161C15.2228 13.3047 16.4892 13.9251 16.7464 14.0531C16.7972 14.0784 16.8448 14.1012 16.8889 14.1224C17.0678 14.2082 17.1895 14.2665 17.2411 14.3535C17.3054 14.4618 17.3054 14.9739 17.0927 15.5746C16.8751 16.1752 15.8263 16.7513 15.3514 16.7956C15.3064 16.7999 15.2617 16.8053 15.2156 16.8108C14.7804 16.8635 14.228 16.9303 12.2596 16.1555C9.83424 15.2018 8.23322 12.8354 7.90953 12.357C7.88398 12.3192 7.86638 12.2932 7.85698 12.2806L7.8515 12.2733C7.70423 12.0762 6.80328 10.8707 6.80328 9.62685C6.80328 8.43682 7.38951 7.81726 7.65689 7.53467C7.67384 7.51676 7.6895 7.50021 7.70366 7.48494C7.94107 7.22895 8.21814 7.16495 8.39125 7.16495C8.56445 7.16495 8.73756 7.16495 8.88595 7.16985Z" fill="currentColor" />
        <path fillRule="evenodd" clipRule="evenodd" d="M2.18418 21.3314C2.10236 21.6284 2.37285 21.9025 2.6709 21.8247L7.27824 20.6213C8.7326 21.409 10.37 21.8275 12.0371 21.8275H12.0421C17.5281 21.8275 22 17.3815 22 11.9163C22 9.26735 20.966 6.77594 19.0863 4.90491C17.2065 3.03397 14.7084 2 12.042 2C6.55607 2 2.08411 6.44605 2.08411 11.9114C2.08348 13.65 2.5424 15.3582 3.41479 16.8645L2.18418 21.3314ZM4.86092 17.2629C4.96774 16.8752 4.91437 16.4608 4.71281 16.1127C3.97266 14.8348 3.58358 13.3855 3.58411 11.9114C3.58411 7.28158 7.37738 3.5 12.042 3.5C14.3119 3.5 16.4296 4.37698 18.0281 5.96805C19.6248 7.55737 20.5 9.66611 20.5 11.9163C20.5 16.5459 16.7068 20.3275 12.0421 20.3275H12.0371C10.6206 20.3275 9.22863 19.9718 7.99266 19.3023C7.65814 19.1211 7.26726 19.0738 6.89916 19.17L4.13676 19.8915L4.86092 17.2629Z" fill="currentColor" />
      </svg>
    </a>
  );
};

const ThemeShopLayout: React.FC<Props> = async ({ children, header, locale }) => {
  const headerData: HeaderProps = await getCachedGlobal("header" as keyof Config["globals"], 1, locale)();
  const settingsData: Settings = await getCachedGlobal("settings" as keyof Config["globals"], 1, locale)();
  const salesChannel = await getSalesChannel();

  if (!salesChannel) {
    return <div>No sales channel found</div>;
  }
  const currency = await getCurrentCurrency(salesChannel);
  const currencies = await getCurrencies(salesChannel);

  const enabledLocales = settingsData.locales as string[] | undefined;
  const enabledLocalesArray = LOCALES.filter((l) => {
    const code = typeof l === "string" ? l : l.code;
    return enabledLocales ? enabledLocales.includes(code) : true;
  });

  const dictionary = await getDictionary(locale);
  const enableWhatsappChat = settingsData.enableWhatsappChat || false;

  return (
    <EcommerceProvider locale={locale} currency={currency} salesChannel={salesChannel} dictionary={dictionary}>
      <div>
        <div className="relative sticky top-0 z-30 text-black bg-white dark:bg-black lg:block dark:text-white">{header ? header : <Header {...headerData} currencies={currencies} currentCurrency={currency} locales={enabledLocalesArray} />}</div>
        {children}
        <Footer locale={locale} />
        {enableWhatsappChat && settingsData.whatsappChatLink ? <WhatsappChat url={settingsData.whatsappChatLink} /> : null}
      </div>
      <Toaster />
    </EcommerceProvider>
  );
};

export { ThemeShopLayout };
