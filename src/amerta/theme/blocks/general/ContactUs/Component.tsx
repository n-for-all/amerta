...
import type { ThemeShopContactUsBlock as ContactUsBlockProps, Media } from "@/payload-types";
...
export const ThemeShopContactUsBlock: React.FC<Props> = async ({ className, titlePrefix, titleSuffix, image, contactDetails, socialLinks, form, locale }) => {
  const imgUrl = typeof image === "object" ? (image as Media)?.url : null;
  const imgAlt = typeof image === "object" ? (image as Media)?.alt : "Contact image";
      ...
      <div className="flex flex-col justify-between md:gap-14 lg:flex-row">
        ...
        <div className="flex flex-col gap-6 overflow-hidden flex-2/3 md:gap-24 lg:gap-28 xl:gap-32">
          ...
          <div className="mt-auto flex flex-col gap-8 sm:flex-row lg:gap-6 xl:gap-2.5">
            <div className="flex-1/3 xl:flex-1/3">{imgUrl && <ImageMedia alt={imgAlt!} src={imgUrl} width={494} height={529} imgClassName="object-cover w-full h-auto" />}</div>

            <div className="flex flex-1/3 sm:justify-center xl:flex-2/3">
              <div className="flex flex-col self-center max-w-sm gap-7">
                {contactDetails?.email && (
                  <div>
                    <p data-slot="text" className="uppercase text-zinc-500 text-sm/6">
                      {__("Email")}
                    </p>
                    <p data-slot="text" className="mt-2 uppercase text-sm/6">
                      {contactDetails.email}
                    </p>
                  </div>
                )}
            ...
            <div className="flex w-full flex-1/3 lg:self-center">
              {form && typeof form === "object" && (
                <div className="flex flex-col w-full max-w-sm gap-5">
                  <FormBlock
                    form={form as any}
                    enableIntro={false}
                    className="contact-us-form" // Add this class to target CSS if needed
                  />
                </div>
              )}
            </div>
...
// Helper component to render icons based on selection
const SocialIcon = ({ platform, url }: { platform: string; url: string }) => {
  ...
  return (
    <a className="text-zinc-600 hover:text-zinc-800" href={url} target="_blank" rel="noopener noreferrer">
