...
import { ADMIN_EMAILS } from "@/amerta/collections/EmailTemplates";
import { Customer, User } from "@/payload-types";

export async function generateEmailTemplate(name: string) {
  ...
  const sendTemplate = async ({ customer, props }: { customer: Customer; props?: Record<string, any> }) => {
    ...
    const subject = nunjucks.renderString(templateDoc.subject, mergedContext);
    const html = nunjucks.renderString(templateDoc.body, mergedContext);
    const to = ADMIN_EMAILS.includes(name) ? ((templateDoc.staffRecipients || []) as User[]).map((r: User) => r.email).join(",") || "" : customer.hasAccount === "1" ? customer.email : customer.contact_email || "";
